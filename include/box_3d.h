...
public:
    ...
    Box3D(T* data , const int& pnum) : Box3D(){
        for(int i = 0 ; i < pnum ; i ++){
            x_min_ = std::min(x_min_, data[3*i+0]);
            x_max_ = std::max(x_max_, data[3*i+0]);
            y_min_ = std::min(y_min_, data[3*i+1]);
            y_max_ = std::max(y_max_, data[3*i+1]);
            z_min_ = std::min(z_min_, data[3*i+2]);
            z_max_ = std::max(z_max_, data[3*i+2]);            
        }
    }
